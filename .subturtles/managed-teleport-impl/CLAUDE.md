...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after hardening local hosted session file handling to require a regular file on disk. Most recently, `readSession()` / permission hardening now reject symlinked or directory-backed session paths instead of following them, so hosted session reuse stays pinned to an expected local file contract.
