...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after pinning hosted browser verification URLs to the configured control-plane origin. Most recently, login-start validation now rejects `verification_uri` and `verification_uri_complete` values that do not stay on the configured control-plane origin, so browser sign-in cannot bounce users onto an unrelated host.
