...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after hardening local hosted session file handling to require a regular file on disk. Most recently, hosted session path inspection now uses `lstat`-based existence checks so dangling symlink session paths are rejected during both session reuse and login writes instead of being treated as missing files.
