...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after extending on-read legacy session normalization to backfill pinned control-plane and cached snapshot freshness metadata.
