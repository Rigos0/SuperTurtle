...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after extending fail-closed validation across both control-plane payloads and on-read local hosted session state.
