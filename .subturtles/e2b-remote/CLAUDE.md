## Current task
Create `super_turtle/e2b/remote.sh` command router (`up|status|pause|resume|stop|sync|reconcile-cron`) with strict error handling.
...
- [ ] Create `super_turtle/e2b/remote.sh` command router (`up|status|pause|resume|stop|sync|reconcile-cron`) with strict error handling <- current
- [ ] Implement sandbox state manager in `super_turtle/e2b/state.sh` (read/write `super_turtle/e2b/.state.json`, validation, stale-state recovery)
