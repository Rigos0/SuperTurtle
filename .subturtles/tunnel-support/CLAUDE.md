...
- [ ] Update META_SHARED.md: on cron check-ins, check for `.tunnel-url` in the SubTurtle workspace. If found and not yet sent to user, send the link. Document that SubTurtles doing frontend work should have "start tunnel" as their first backlog item. <- current
