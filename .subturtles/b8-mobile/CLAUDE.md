## Current Task
Mobile-fix profile in `linkedin-demo/src/components/profile/Style.js` + `Profile.js`: ensure profile header, tabs, and post list don't overflow; make cover image responsive; ensure back button is visible and tappable.
...
- [ ] Mobile-fix profile in `linkedin-demo/src/components/profile/Style.js` + `Profile.js`: Ensure the profile header, tabs, and post list don't overflow. Profile cover image should be responsive. Back button should be visible and tappable. <- current
