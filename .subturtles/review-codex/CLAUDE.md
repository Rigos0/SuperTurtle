# Current task
Review `super_turtle/subturtle/` — Python loop runner, ctl CLI, helpers. Add any findings to `reviews/codex-review.md`.
...
# Backlog
- [x] Review `super_turtle/claude-telegram-bot/src/` — all TypeScript source files
- [ ] Review `super_turtle/subturtle/` — Python loop runner, ctl CLI, helpers <- current
- [ ] Review `super_turtle/meta/` — prompt files for inconsistencies or stale references
- [ ] Review root config files (CLAUDE.md, package.json, tsconfig, etc.)
- [ ] Write final `reviews/codex-review.md` with prioritized findings
