...
- [x] Fix docs/bot/drivers.mdx: read codex-session.ts and config.ts to get actual model names. Replace speculative model lists with real ones. Update any "Claude 4.0" references.
- [ ] Review docs/bot/overview.mdx: verify features list against current handlers. Remove "Demo GIF coming soon" if present. Ensure voice/photo/document handling descriptions match current code. <- current
