## Current Task
Update `YOLO_PROMPT` with self-stop instruction to append `## Loop Control\nSTOP` after committing when backlog is fully complete, then amend the commit.
...
- [ ] Update `YOLO_PROMPT` — add step 6 telling the agent to append `## Loop Control\nSTOP` to the state file after committing when ALL backlog items are `[x]`, then amend the commit. <- current
