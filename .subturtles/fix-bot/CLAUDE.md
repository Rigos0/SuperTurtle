# Current task
Fix #6 in `super_turtle/claude-telegram-bot/src/handlers/streaming.ts`: remove the unused `PINO_LOG_PATH` import and keep `streamLog`.
...
- [ ] Fix #6: streaming.ts line 27 — remove unused `PINO_LOG_PATH` from the import. Keep `streamLog`. <- current
