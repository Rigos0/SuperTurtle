## Current Task
Fix `linkedin-demo/src/convex/likes.ts` setReaction + removeReaction — use `getAuthUserId(ctx)` instead of `args.userId`.
...
- [ ] Fix `linkedin-demo/src/convex/likes.ts` setReaction + removeReaction (line ~82): same auth fix <- current
