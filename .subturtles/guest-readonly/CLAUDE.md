...
- [ ] Update `linkedin-demo/src/components/posts/post/Post.js`: for unauthenticated users (no user._id): disable the like button (grey it out), hide the comment input field, hide the edit/delete menu. The post content remains fully visible and readable. <- current
...
- App.js at `linkedin-demo/src/App.js` — now always renders feed layout and opens `LoginCard` in a modal when `showLogin` is true
