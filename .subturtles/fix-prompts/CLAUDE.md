# Current task
Fix #22: change `--append-system-prompt` to `--system-prompt` in `super_turtle/meta/claude-meta` for consistency with the bot. Commit after the fix.
...
- [ ] Fix #22: super_turtle/meta/claude-meta line 34 — change `--append-system-prompt` to `--system-prompt` for consistency with the bot (session.ts line 549 uses `--system-prompt`). META_SHARED.md is designed to be self-contained. <- current
