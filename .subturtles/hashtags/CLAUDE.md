## Current Task
Create `linkedin-demo/src/convex/hashtags.ts` — query: getPostsByHashtag(tag) returns posts with that hashtag.
...
- [ ] Create `linkedin-demo/src/convex/hashtags.ts` — query: getPostsByHashtag(tag) returns posts with that hashtag <- current
