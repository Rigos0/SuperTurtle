...
- [ ] Fix network cards dark mode in `linkedin-demo/src/components/network/Style.js`: Replace card `backgroundColor: "#fff"` with `theme.palette.background.paper`. Replace search field `backgroundColor: "#fff"` with `theme.palette.background.paper`. <- current
