## Current Task
Integrate autocomplete into post composer.
...
- [ ] Integrate autocomplete into post composer — `linkedin-demo/src/components/posts/postMaker/PostMaker.js` — detect @ in textarea, show dropdown, insert @username on select <- current
- [ ] Render @mentions as styled links in post text — in Post.js, regex-replace @username with clickable link to `/:username`
- [ ] Create notification on mention — in createPost mutation (`linkedin-demo/src/convex/posts.ts`), parse @usernames, create notification for each mentioned user
