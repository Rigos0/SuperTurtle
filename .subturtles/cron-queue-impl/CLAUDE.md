# Current task
Update `src/index.ts` cron timer so busy non-silent cron jobs are enqueued with `enqueueDeferredCronJob()` instead of skipped, without removing/advancing the cron record at enqueue time.
