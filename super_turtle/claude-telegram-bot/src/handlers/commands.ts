    ...
    const child = Bun.spawn(
      [
        ...
        [
          ...
          "  fi",
          '  if [ -d "$SUPERTURTLE_TELEPORT_CLAIM_DIR" ]; then',
          '    rmdir "$SUPERTURTLE_TELEPORT_CLAIM_DIR" 2>/dev/null || true',
          "  fi",
          "}",
          "acquire_lock() {",
          "  local attempt",
          '  for attempt in 1 2; do',
          '    if mkdir "$SUPERTURTLE_TELEPORT_CLAIM_DIR" 2>/dev/null; then',
          "      return 0",
          "    fi",
          '    existing_pid=""',
          ...
          '    rmdir "$SUPERTURTLE_TELEPORT_CLAIM_DIR" 2>/dev/null || true',
          "  done",
          '  printf \'[teleport] failed to acquire managed teleport lock after stale-lock cleanup\\n\' >>"$SUPERTURTLE_TELEPORT_LOG_PATH"',
          "  exit 1",
          "}",
          'umask 077',
          "acquire_lock",
      ...
      ],
      {
        ...
        env: {
          ...
          SUPERTURTLE_TELEPORT_CLAIM_DIR: TELEPORT_CLAIM_DIR,
